...
from sklearn.model_selection import cross_val_score
accuracies = cross_val_score(estimator = gnb, X = features_test, y = labels_test, cv = 10)
model_accuracy.append(accuracies.mean())
print ("mean accuracy is",accuracies.mean())
print (accuracies.std())
...
rects=plt.bar(list2,model_accuracy)
plt.xlabel('Different Models')
plt.ylabel('Accuracy')
plt.title('Baseline Accuracy of Different Models')
plt.grid(True)
def autolabel(rects):
    """
    ...
    for rect in rects:
        ...
        plt.text(rect.get_x() + rect.get_width()/2., .55*height,'%f' % float(height),ha='center', va='bottom')
